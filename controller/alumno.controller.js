...
export default {
  ...
  eliminar:async(req, res, next)=>{
      try {
        ...
        const eliminarPersona= await model.Alumno.findByIdAndDelete(req.params.id);
        res.status(200).send({
  ...
  Actualizar: async(req, res, next)=>{
      try {
          ...
          const datosActuales = await model.Alumno.findByIdAndUpdate(req.params.id, actualizarPersona);
